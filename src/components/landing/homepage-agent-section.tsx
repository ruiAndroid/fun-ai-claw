  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
      {Array.from({ length: 6 }).map((_, index) => (
        <div
          key={index}
          className="h-full min-h-[380px] overflow-hidden rounded-[28px] border border-white/70 bg-white/84 p-6 shadow-[0_24px_60px_rgba(81,38,145,0.08)]"
        >
...
export function HomepageAgentSection({
  ...
  const sectionDescription = useMemo(() => {
    if (loading) {
      return "正在为你加载已正式上线的机器人，稍后即可选择体验。";
    }
    if (error) {
      ...
      {!loading && !error && agents.length > 0 ? (
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {agents.map((agent, index) => {
            const hasAgentTags = Boolean(agent.model || agent.provider || agent.agentic);
            ...
            return (
              ...
              >
                <motion.article
                  ...
                  className="group relative flex h-full min-h-[380px] w-full cursor-pointer overflow-hidden rounded-[30px] border border-white/70 bg-white/88 p-6 shadow-[0_24px_60px_rgba(81,38,145,0.08)] backdrop-blur-xl transition-shadow duration-300 hover:shadow-[0_30px_70px_rgba(139,61,255,0.14)]"
                >
                  ...
                  <div className="relative flex h-full w-full flex-col">
                    ...
                    <div className="mt-5">
                      <h3
                        className="text-xl font-bold tracking-tight text-md-on-surface"
                    ...
                    {hasAgentTags ? (
                      <div className="mt-3 flex flex-wrap content-start gap-2">
                        {agent.model ? (
                          <span className="inline-flex max-w-full items-center gap-1 rounded-full border border-md-outline-variant/40 bg-white/72 px-3 py-1 text-xs font-semibold text-md-on-surface-variant">
                    ...
                    <div className={`${hasAgentTags ? "mt-8" : "mt-7"} w-full rounded-[20px] border border-md-outline-variant/25 bg-white/72 px-4 py-4`}>
                      <div className="text-[11px] font-semibold text-md-on-surface-variant">简介</div>
                      <div className="mt-2 min-h-[3rem] w-full">
                        <div
                          className="w-full text-sm leading-6 text-md-on-surface"
                          style={twoLineClampStyle}
                        >
                    ...
                    <div className="mt-auto flex items-center justify-between gap-3 pt-6">
                      ...
                      <span className="inline-flex shrink-0 items-center gap-2 rounded-full bg-[linear-gradient(135deg,#ff7a18_0%,#ff9f43_42%,#8b3dff_100%)] px-4 py-2 text-sm font-semibold text-md-on-primary shadow-[0_14px_28px_rgba(139,61,255,0.16)] transition-all duration-300 hover:shadow-[0_18px_36px_rgba(139,61,255,0.22)]">
